...
import com.ayfp.anyuanwisdom.nim.Extras;
    ...
    public void activeCallingNotification(boolean active) {
        if (notificationManager != null) {
            if (active) {
                ...
                NimCache.getNotifications().put(CALLING_NOTIFY_ID, callingNotification);
            } else {
                notificationManager.cancel(CALLING_NOTIFY_ID);
                NimCache.getNotifications().remove(CALLING_NOTIFY_ID);
            }
        }
    }
    ...
    public void activeMissCallNotification(boolean active) {
        if (notificationManager != null) {
            if (active) {
                ...
                NimCache.getNotifications().put(MISS_CALL_NOTIFY_ID, callingNotification);
            } else {
                notificationManager.cancel(MISS_CALL_NOTIFY_ID);
                NimCache.getNotifications().remove(MISS_CALL_NOTIFY_ID);
            }
        }
    }
}
