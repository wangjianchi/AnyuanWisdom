...
import com.ayfp.anyuanwisdom.nim.avchat.widgets.ToggleListener;
import com.ayfp.anyuanwisdom.nim.avchat.widgets.ToggleState;
import com.ayfp.anyuanwisdom.nim.avchat.widgets.ToggleView;
...
public class AVChatAudio implements View.OnClickListener, ToggleListener {
    ...
    public void showNetworkCondition(int grade) {
        if (grade >= 0 && grade < NETWORK_GRADE_DRAWABLE.length) {
            ...
            Drawable drawable = NimCache.getContext().getResources().getDrawable(NETWORK_GRADE_DRAWABLE[grade]);
            if (drawable != null) {
    ...
    private void setWifiUnavailableNotifyTV(boolean show) {
        if (show && !NetworkUtil.isWifi(NimCache.getContext())) {
